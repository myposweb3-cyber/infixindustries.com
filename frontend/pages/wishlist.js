...
export default function WishlistPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      bc.onmessage = (ev) => { if (ev.data?.key === 'wishlist') read() }
    } catch (e) {}
  ...
  function remove(index) {
    const raw = JSON.parse(localStorage.getItem('wishlist') || '[]')
    raw.splice(index, 1)
    localStorage.setItem('wishlist', JSON.stringify(raw))
    try { new BroadcastChannel('infixsite').postMessage({ key: 'wishlist' }) } catch(e){}
    read()
  }
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)] py-20">
      <div className="mx-auto max-w-4xl px-6">
        <div className="rounded-[28px] border border-slate-200 bg-white p-10 text-center">
          ...
          <p className="mt-3 text-slate-600">Add items to your wishlist to see them here.</p>
