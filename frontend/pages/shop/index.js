...
export default function Shop() {
  ...
  return (
    <div key={p.id} className="group relative overflow-hidden rounded-[24px] bg-gradient-to-b from-slate-900 to-slate-950 shadow-xl transition-all duration-300 hover:shadow-2xl hover:shadow-yellow-400/10">
      ...
      <div className="flex flex-col gap-3 p-5">
        <h3 className="min-h-[3rem] text-lg font-semibold leading-tight text-white line-clamp-2 group-hover:text-yellow-300 transition-colors">{p.title}</h3>
        <div className="flex items-baseline gap-2">
          <p className="text-2xl font-bold text-yellow-400">{formatMoney(p.discount_price || p.price || 0)}</p>
          {discount > 0 && <p className="text-sm text-gray-400 line-through">{formatMoney(p.price || 0)}</p>}
        </div>
        <div className="h-px bg-gradient-to-r from-yellow-400/20 via-yellow-400/10 to-transparent"></div>
        <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:gap-2">
          <Link href={`/product/${p.slug || p.id}`} className="flex-1 rounded-full border-2 border-yellow-400 px-4 py-2.5 text-center text-sm font-semibold text-yellow-400 transition-all duration-300 hover:bg-yellow-400 hover:text-slate-900 hover:shadow-lg hover:shadow-yellow-400/30">View</Link>
          <button onClick={() => addToCart(p)} disabled={addingProductId === p.id} className="flex-1 rounded-full bg-gradient-to-r from-yellow-400 to-yellow-500 px-4 py-2.5 text-center text-sm font-bold text-slate-900 shadow-lg shadow-yellow-400/30 transition-all duration-300 hover:shadow-xl hover:shadow-yellow-400/50 hover:from-yellow-300 hover:to-yellow-400 disabled:opacity-70 disabled:cursor-not-allowed">{addingProductId === p.id ? 'Adding...' : 'Add Cart'}</button>
        </div>
      </div>
      <div className="absolute inset-0 rounded-[24px] border border-yellow-400/0 transition-all duration-300 group-hover:border-yellow-400/20 pointer-events-none"></div>
    </div>
  );
})}</div><div className="mt-8 flex justify-center gap-3"><button onClick={() => page > 1 && setPage(page - 1)} disabled={page === 1} className="rounded-full border-2 border-yellow-400/50 px-6 py-3 font-semibold text-yellow-400 transition-all hover:border-yellow-400 hover:bg-yellow-400/10 disabled:opacity-50 disabled:cursor-not-allowed">← Previous</button><span className="flex items-center gap-2 px-4 font-semibold text-gray-300">Page {page}</span><button onClick={() => page * 12 < total && setPage(page + 1)} disabled={page * 12 >= total} className="rounded-full border-2 border-yellow-400/50 px-6 py-3 font-semibold text-yellow-400 transition-all hover:border-yellow-400 hover:bg-yellow-400/10 disabled:opacity-50 disabled:cursor-not-allowed">Next →</button></div></main></div></section></div>
}
